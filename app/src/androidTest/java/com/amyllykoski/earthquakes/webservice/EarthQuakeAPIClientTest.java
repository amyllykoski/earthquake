...
public class EarthQuakeAPIClientTest {
  ...
  private class Helper implements EarthResponseReceiver {
    ...
    List<EarthQuakeRecord> getRecords() {
      return mRecords;
    }
  ...
  public void should_receiveResponseCorrectly() throws Exception {
    ...
    Helper helper = runTest(okResponse);
    assertEquals(3, helper.getRecords().size());
  }

  @Test
  public void should_receiveOnFailureDueToBrokenJSON() throws Exception {
    String brokenJSON = "{\"type\":\"FeatureCollection\",";
    Helper helper = runTest(brokenJSON);
    assertEquals(0, helper.getRecords().size());
    assertEquals("Failed to communicate with USGS Earthquakes.", helper.getFailureMessage());
    assertTrue(!helper.getFailureDetails().isEmpty());
  }

  @Test
  public void should_receiveOnFailureDueToEmptyJSON() throws Exception {
    String emptyResponse = "";
    Helper helper = runTest(emptyResponse);
    assertEquals(0, helper.getRecords().size());
    assertEquals("Failed to communicate with USGS Earthquakes.", helper.getFailureMessage());
  }

  @NonNull
  private Helper runTest(String brokenJSON) throws IOException, InterruptedException {
